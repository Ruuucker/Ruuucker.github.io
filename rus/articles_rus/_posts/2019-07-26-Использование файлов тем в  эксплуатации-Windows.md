...
Возьмем пример с тремя шрифтами. Есть три шрифта, потому что один использует только Windows 7 x86, другой использует Windows 7 x64, третий использует Windows XP x86. Обычно, чтобы получить полное покрытие, вы должны поместить все три файла шрифтов в каталог. Когда цель переходит в каталог, содержащий файлы шрифтов, выполняется выполнение. Теперь, допустим, у вас есть еще один из «интересных» файлов Microsoft, таких как desktop.ini или library-ms. Вы можете использовать другие файлы, чтобы указать, что ваш файл темы должен быть значком для чего-то. Для этого примера предположим, что мы делаем его значком чего-то в корне диска. Мы разместим систему шрифтов, спрятанную и похороненную на диске. Когда цель попадает в корень диска и отображается значок, файл темы отображается. Затем файл темы имеет относительные пути к файлам шрифтов в системе и отображает все три шрифта. Таким образом, вы перешли от необходимости использования трех эксплойтов в каталоге, к которому ведет цель, к скрытым файлам, сохраняя при этом тот же охват.
